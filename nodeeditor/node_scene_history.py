...
class SceneHistory():
    ...
    def restoreHistory(self):
        logging.debug(f"Restoring history.... current_step: {self.history_current_step} of {len(self.history_stack)}")
    ...
    def storeHistory(self, desc, setModified=False):
        ...
        logging.debug(f"Storing history..{desc}, current_step: {self.history_current_step} of {len(self.history_stack)}")
        ...
        self.history_current_step += 1
        logging.debug(f"  -- setting step to: {self.history_current_step}")
    ...
    def restoreHistoryStamp(self, history_stamp):
        logging.debug(f"RHS: {history_stamp['desc']}")
