...
class QDMGraphicsView(QGraphicsView):
    ...
    def middleMouseButtonPress(self, event):
        ...
        if DEBUG:
            if isinstance(item, QDMGraphicsEdge):
                logging.debug(f'RMB DEBUG: {item.edge} connecting sockets:'
                              f'{item.edge.start_socket} <--> {item.edge.end_socket}')
            if type(item) is QDMGraphicsSocket:
                logging.debug(f'RMB DEBUG: {item.socket} has edges: {item.socket.edges}')
    ...
    def leftMouseButtonPress(self, event):
        ...
        item = self.getItemAtClick(event)
        logging.debug(f'Left mouse click on {item}')
        #if item is None:
        #    pass
    ...
    def edgeDragStart(self, item):
        try:
            ...
            logging.debug('View::edgeDragStart ~   assign Start Socket to: {item.socket}')
            self.drag_start_socket = item.socket
            self.drag_edge = Edge(self.grScene.scene, item.socket, None, EDGE_TYPE_BEZIER)
            logging.debug(f'View::edgeDragStart ~   dragEdge: {self.drag_edge}')
        ...
        try:
            if type(item) is QDMGraphicsSocket:
                if item.socket != self.drag_start_socket:
                    ...
                    logging.debug(f"View::edgeDragEnd ~ created new edge: {new_edge}, "
