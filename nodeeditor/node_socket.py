...
class Socket(Serializable):
    def __init__(self,
        ...
        #logging.debug(f"Socket -- creating with index {self.index}, pos {self.position} for node {self.node.__class__.__name__}")
...
#        logging.debug(f"  GSP: {self.index}, {self.position}, node: {self.node}")
...
#        logging.debug(f"  res {res}")
    ...
    def removeEdge(self, edge):
        if edge in self.edges:
            ...
            logging.debug(f'wanna remove edge {edge}, from self.edges but it\'s not in the list!')
